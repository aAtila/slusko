...
- **The Summary in the database is not regenerated by speaker-mapping
  changes.** It is written once by the pipeline. User edits to the Summary
  (overview, decisions, action items, open questions) are allowed and are
  last-write-wins — v1 has no versioning, audit log, or per-user
  attribution. What never re-runs is the **LLM call**: speaker mapping
  changes apply at render time only and never trigger a regenerate.
