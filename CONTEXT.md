...
The Summary is generated by the pipeline and stored as the Meeting's current
Summary. For the issue #28 starter slice, Slusko does not keep version history,
revision instructions, reset provenance, or durable regeneration-attempt history.

The Transcript, in contrast, is **read-only** in v1. People correct summaries
more often than transcripts in tools like this, and keeping the transcript
immutable also keeps the door closed on the "regenerate the summary from an
edited transcript" cost trap (see
[ADR 0005](./docs/adr/0005-speaker-mapping-is-cosmetic.md)).

### Summary regeneration

A user-triggered rerun of summarization for a done Meeting, using the existing
Transcript as input with raw speaker labels rather than current Speaker mappings.
Summary regeneration is asynchronous post-completion work, separate from the
first-time Meeting pipeline: the Meeting remains done while the current Summary
remains visible, readable, and exportable. The Summary is replaced only after
regeneration succeeds. If regeneration fails, the Meeting stays done, the
current Summary is unchanged, and any user-facing failure notice is ephemeral
rather than persisted as Meeting failure state.

For the starter slice, Slusko tracks only the Meeting's current regeneration
state (`idle`, `pending`, `processing`, or `failed`) and the timestamp needed for
stale processing recovery. Success returns the regeneration state to `idle` after
replacing the Summary. Summary regeneration is available only for done Meetings
that have a Summary, requires confirmation because it replaces the current
Summary, and only one regeneration may be active for a Meeting at a time. While
regeneration is active, the Meeting detail page polls for completion. The current
page may show ephemeral success or failure feedback; failure feedback tells the
user the previous Summary is unchanged. First-time Meeting processing takes
priority over regenerating an already-readable Summary.
...
A Meeting can be exported in two flavors. Exports always use the current Summary.
